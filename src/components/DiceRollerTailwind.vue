...
<template>
  ...
  >
    <div :id="id" class="dice-roller p-4 rounded bg-slate-100 text-gray-800 dark:bg-gray-600 dark:text-gray-200">
      <output
          v-if="output"
          name="output"
          :for="inputId"
      ...
      <span v-if="error" class="block mt-4 text-red-600 text-sm dark:text-red-200">{{ error }}</span>
    </div>
  </DiceRollerRenderless>
</template>
